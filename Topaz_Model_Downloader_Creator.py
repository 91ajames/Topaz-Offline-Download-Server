...
STARLIGHT_FILES = [
    (
        "slp25.zip",
        f"https://video-models.topazlabs.com/slp-2.5/{STARLIGHT_VERSION}/win/slp25.zip",
    ),
]
...
#    f.write('if "%TOPAZ_DOWNLOADS_ONLINE%"=="1" (\n')
    ...
    f.write("echo.\n")
    f.write("echo ===========================================\n")
    f.write("echo          Neuroserver Downloader\n")
    f.write("echo ===========================================\n")
    f.write("echo.\n")
    ...
    f.write("echo.\n")
    f.write("echo.\n")
    f.write("echo        Neuro Downloader Complete\n")
    f.write("echo.\n")
    f.write("echo.\n")
    f.write("echo.\n")
    ...
    for i, (name, url) in enumerate(STARLIGHT_FILES, start=1):
        ...
        f.write("    echo Already exists. Skipping.\n")
        f.write(") else (\n")
        f.write(f'    curl -L --fail --range 0-0 --connect-timeout 5 --max-time 10 -o "%TEMP%\\topaz_starlight_test_{i}.tmp" "{url}" >nul 2>&1\n')
        f.write("    if not errorlevel 1 (\n")
        f.write(f'        curl -L --fail --retry 2 --connect-timeout 5 -o "%MIRROR_ROOT%\\slp-2.5\\{STARLIGHT_VERSION}\\win\\{name}" "{url}"\n')
        f.write("        if errorlevel 1 (\n")
        f.write(f"            echo FAILED: {name}\n")
        f.write(f'            del "%MIRROR_ROOT%\\slp-2.5\\{STARLIGHT_VERSION}\\win\\{name}" >nul 2>&1\n')
        f.write("        )\n")
        f.write("    ) else (\n")
        f.write(f"        echo FAILED: {name}\n")
        f.write("    )\n")
        f.write(f'    del "%TEMP%\\topaz_starlight_test_{i}.tmp" >nul 2>&1\n')
        f.write("    )\n")
        f.write(")\n")
        f.write("echo.\n\n")
    f.write("echo.\n")
    f.write("echo.\n")
    f.write("echo      StarLight 2.5 Downloader Complete\n")
    f.write("echo.\n")
    f.write("echo.\n")
    f.write("echo.\n")
    ...
    f.write("echo ===========================================\n")
    f.write("set STARLIGHT_MISSING_COUNT=0\n")
    f.write("echo.\n")
    ...
    f.write('echo.\n')
    f.write('set "IP_COUNT=0"\n')
    f.write('for /f "tokens=2 delims=:" %%A in (\'ipconfig ^| findstr /i "IPv4"\') do (\n')
    f.write('    set /a IP_COUNT+=1\n')
    f.write('    set "CURRENT_IP=%%A"\n')
    f.write('    set "CURRENT_IP=!CURRENT_IP: =!"\n')
    f.write('    call set "IP_%%IP_COUNT%%=%%CURRENT_IP%%"\n')
    f.write('    echo   !IP_COUNT!^) !CURRENT_IP!\n')
    f.write(')\n')
    f.write('echo.\n')
    ...
    f.write(':CHOOSE_HOST_IP\n')
    f.write('echo.\n')
    f.write('set /p IP_CHOICE=Choose server IP option 1-!IP_COUNT!: \n')
    f.write('if "!IP_CHOICE!"=="" goto CHOOSE_HOST_IP\n')
    f.write('call set "HOST_IP=%%IP_!IP_CHOICE!%%"\n')
    f.write('if "!HOST_IP!"=="" (\n')
    f.write('    echo Invalid option. Try again.\n')
    f.write('    goto CHOOSE_HOST_IP\n')
    f.write(')\n')
    f.write('echo.\n')
    ...
    f.write('echo.\n')
    f.write('echo Choose another IP address.\n')
    f.write('goto CHOOSE_HOST_IP\n\n')
